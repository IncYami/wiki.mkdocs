...
**Contêiner:** `.mkv` e `.mp4`, eles contêm vários fluxos de vídeo, áudio e legenda dentro deles. Mesmo o mesmo vídeo exato pode ser colocado em recipientes diferentes. Você pode converter sem perdas entre eles com [ffmpeg](https://ffmpeg.org/download.html).

**Codec:** Isso é o que a maioria das pessoas quer dizer quando o confunde com o contêiner. O codec é o maior fator de compatibilidade com seu hardware e sistema. HEVC(h265/x265) e AVC(h264/x264) são os principais.

**Profundidade de bits:** 8 bits/10 bits - A maioria das codificações de anime de alta qualidade são de 10 bits. Converter uma fonte de 8 bits em 10 bits pode parecer contra-intuitivo se você souber um pouco sobre transcodificação e perda de dados, [mas dá um resultado melhor em tamanhos menores.](_resources\why_does_10bit_save_bandwidth_-_ateme.pdf)

**Frame rate:** Normalmente será 23,976 FPS. Muitas TVs usam [interpolação](https://en.wikipedia.org/wiki/Motion_interpolation) para transformar isso em 60 fps, dando a você uma sensação artificial de suavidade. Isso não é recomendado para anime e deve ser desativado nas configurações. As codificações de 60 FPS são ainda piores e devem ser evitadas.

**Level/Profile:** São especificações dentro do padrão h264/h265 que dão uma ideia de compatibilidade e especificam a resolução máxima e taxa de bits, por exemplo:

- h264 4.0 = 1080p 30fps 20 Mbps.
- Level/Profile mais alto = menor compatibilidade = mais poder de processamento necessário para decodificar.

---

As informações completas de um vídeo serão semelhantes a:
...
| --------------- | --------------------------------------------------------------------- |
| `.mkv`          | Recipiente comumente usado que mantém tudo junto.                     |
| BD              | O vídeo é obtido de BluRay (normalmente JPBD)                         |
| Remux           | Vídeo intocado retirado de BluRay                                     |
| x265/HEVC/H.265 | Codec usado para codificar o vídeo                                    |
| x264/AVC/H.264  | Codec usado para codificar o vídeo                                    |
| AV1             | Codec usado para codificar o vídeo                                    |
| 1080p           | A resolução é 1920x1080p, `p` significa vídeo progressivo.            |
| 8 bits          | Profundidade de bits do vídeo                                         |
| 10 bits         | Profundidade de bits do vídeo                                         |
| FLAC            | Codec usado para codificar o áudio                                    |
| Opus            | Codec usado para codificar o áudio                                    |
| CAA             | Codec usado para codificar o áudio                                    |
| E-AC-3/EAC3/DDP | Codec usado para codificar o áudio, DDP significa Dobly Digital Plus  |
| Dual Audio      | Possui duas faixas de áudio. Tipicamente Inglês e Japonês             |
| [F0EAA72E]      | A sequência aleatória no final do nome do arquivo geralmente é CRC32. |
